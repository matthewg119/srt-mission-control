...
const STEP_LIST = [
  ...
  // ‼️ `presence_pdf` STOOD HERE AND `findings_doc` STOOD BELOW, UNTIL THE CALL PACK (2026-09-12).
  // Both are now generated by the `call_sheet` runner and filed against that step. Matthew: "we
  // can merge everything that goes in the call pack and keep the rest." Nothing else moved.
  //
  // ‼️ THE ORPHANED ROWS ARE DELETED AFTER THE DEPLOY, NOT BEFORE IT
  // (docs/2026-09-12-call-pack-orphans.sql). loadRows and reachableCursor both re-seed when a
  // client has fewer rows than this array has steps, so deleting two rows while the old code is
  // still live puts them straight back and lets the old runners fire again.
  ...
  //
  // blockedBy inherits what the merged steps waited on, because the RUNNER still needs it:
  // `presence_sweep_manual` because the presence PDF reports the sweep, and `review_audit`
  // because findings section 3 is built from it. Dropping either would let the pack run early and
  // file a document whose own cover says it has nothing to report.
  { key: "call_sheet", phase: PHASE_BEFORE, label: "Call pack: call sheet, findings, presence PDF, closing questions", auto: true, mode: "auto", blockedBy: ["presence_sweep_manual", "review_audit", "custom_question_set", "page_candidates", "hub_preview", "pre_call_pages"] },
