 ...
 function watchAudit(scanId,domain){
  ...
  function tick(){
   ...
   post('/api/concierge/action',{token:token,action:'audit_status',scanId:scanId}).then(function(d){
    ...
    if(d.reportUrl){done=true;teach.stop();fill.style.width='100%';var r=bubble('a','Your AI visibility report is ready.');link(r,d.reportUrl,'Open my report');afterAudit();height();return}
