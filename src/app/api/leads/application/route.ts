  ...
  try {
        ...
        if (!contactId && (email || businessPhone)) {
                  try {
                    ...
                    if (existing) {
                                  // Existing contact — reuse, do NOT push duplicate to Zoho
                                contactId = existing.id;
                    } else {
                                  // Brand new contact — insert into Supabase
            ...
            if (contactId && !dealId) {
                        try {
                          ...
                          await supabaseAdmin.from("deal_events").insert({
                                      ...
                                      console.error("[25%] Deal creation error:", error instanceof Error ? error.message : error);
                        }
            }
            ...
            if (businessName) {
                        ...
                        microsoft.createDriveFolder("Working Files")
                          ...
                          .then(() => console.log(`[25%] OneDrive folder pre-created: Working Files/${safeName}`))
        ...
        // Non-blocking Zoho upsert: search first, update or create
        (async () => {
                  try {
                    ...
                    if (existingZohoLead && existingZohoLead.id) {
                                  // Update existing Zoho lead with all fields
                                await zohoUpdateLead(existingZohoLead.id as string, {
                                                ...
                                                Ownership_Percentage: ownership ? String(ownership) : undefined,
                                                Date_of_Birth: dob,
                                });
        ...
        try {
            ...
            // Attach PDF to Zoho lead (non-blocking, wait up to 3s for zohoLeadId)
            if (pdfBuffer) {
                        // Give the async Zoho create/update a moment to complete before attaching
                    await new Promise(resolve => setTimeout(resolve, 2000));
                        if (zohoLeadId) {
                                      ...
                                      console.warn("[100%] zohoLeadId not available yet — skipping Zoho PDF attachment");
                        }
            }
