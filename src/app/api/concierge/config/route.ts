...
const publicConfig = unstable_cache(
  async (slug: string, category: string | null): Promise<PublicConfig | null> => {
    ...
    return {
      ...
      ctaLabel: config.audience === "owner" ? "Check my visibility" : "Start my free scan",
    };
  },
...
export async function GET(req: NextRequest) {
  ...
  const config = slug ? await publicConfig(slug, category) : null;
