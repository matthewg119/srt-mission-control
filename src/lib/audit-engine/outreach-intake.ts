...
/** Shared no-sell constraints for the intake drafter, kept in sync with PRE_PITCH_RULES. */
const INTAKE_DRAFT_SYSTEM = [
  ...
  "1. NO URLs or links of any kind. Not the report link, not the redesign link, not a calendar link. The redesign, if one exists, is the REWARD for saying yes: hint that you found something on their site working against them, and hold the link back.",
  "2. NO price, no package, no monthly figure.",
  "3. NO meeting ask, NO call ask, NO video ask. The only thing this email asks for is permission to send the breakdown over.",
  "4. Exactly ONE finding. Not three facts. A stranger who reads two findings reads a pitch.",
  "5. Under 120 words for the body. Plain text as if typed in Outlook. No bullets, no bold, no headers.",
  ...
  "Write like the person who personally ran this audit, not a template. Vary sentence length. No 'I hope this finds you well'.",
  "Never guarantee customers, patients, sales or revenue. Never invent a statistic, a screenshot or a competitor name that is not in the findings given.",
...
export async function draftFromIntake(
  ...
  return {
    draft: {
      ...
      subject: stripLinks(noDashes(data.subject)).trim(),
      body: stripLinks(noDashes(data.body)).trim(),
    },
    extracted: {
      prospect_name: cleanText(data.prospect_name),
      prospect_email: cleanText(data.prospect_email),
      redesign_url: cleanText(data.redesign_url),
    },
  };
}
...
export async function revisePreviousDraft(
  ...
  const isPermissionStage = report.outreach_stage !== "revealed";
  ...
  const { data } = await callClaudeJSON<{ subject: string; body: string }>({
    ...
    system: [
      ...
      isPermissionStage
        ? "This email is in the PERMISSION stage, so the constraints still hold no matter what: no URLs or links of any kind, no price, no meeting or call or video ask, exactly one finding, under 120 words. Its only ask is permission to send the breakdown over."
      ...
      "Never guarantee customers or revenue. Never invent a statistic or a competitor name not present in the findings.",
  ...
  const subject = noDashes(data.subject).trim();
  const body = noDashes(data.body).trim();
  return {
    label: previous.label,
    subject: isPermissionStage ? stripLinks(subject) : subject,
    body: isPermissionStage ? stripLinks(body) : body,
  };
}
