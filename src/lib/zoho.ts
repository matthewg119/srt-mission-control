...
export interface ZohoApiRecord {
  ...
  Credit_Score_Range?: string;
        ...
        // Free-form description for extra context
  Description?: string;
        [key: string]: unknown;
}
...
/** Build a full ZohoApiRecord from ZohoLeadData, mapping every available field */
function buildRecord(leadData: ZohoLeadData): ZohoApiRecord {
  ...
  // Put DBA and extra details in Description as fallback
  const descParts: string[] = [];
        if (leadData.dba) descParts.push(`DBA: ${leadData.dba}`);
        if (leadData.useOfFunds) descParts.push(`Use of Funds: ${leadData.useOfFunds}`);
        if (leadData.existingLoans) descParts.push(`Existing Loans: ${leadData.existingLoans}`);
        if (descParts.length > 0) record.Description = descParts.join(" | ");
