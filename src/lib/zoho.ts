...
async function zohoRequest(
  ...
  const json = await response.json();
    // DIAGNOSTIC: log every raw Zoho API response so 200-with-error-body is visible
  console.log("ZOHO RAW RESPONSE:", JSON.stringify(json));
    return json;
}
...
export async function createLead(leadData: ZohoLeadData): Promise<string | null> {
    const record: ZohoApiRecord = {
          ...
          // FIX: Lead_Status was in interface but never mapped to the API record
          Lead_Status: leadData.Lead_Status || "New",
    };

  // DIAGNOSTIC: log what we are about to send to Zoho
  console.log("ZOHO CREATE LEAD PAYLOAD:", JSON.stringify({ data: [record] }));
  ...
  const created = result.data?.[0];
    // DIAGNOSTIC: log individual record result
  console.log("ZOHO CREATE LEAD RESULT:", JSON.stringify(created));

  if (created?.status === "success") {
        return created.details.id;
  }

  // FIX: surface the full error details instead of just message
  throw new Error(
